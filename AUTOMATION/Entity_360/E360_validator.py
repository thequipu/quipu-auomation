...
def validate_schema(schema_path):
    ...
    for pid in all_path_ids:
        ...
        # Build adjacency list (undirected connectivity check)
        for src, tgt, rel in edges:
            graph[src].append(tgt)
            graph[tgt].append(src)
        ...
        if graph:
            ...
            # nodes in path but not reached from start
            disconnected_nodes = [
                n for n in path_to_nodes[pid] if n not in visited
            ]

        # nodes listed in pathIds but not in schema
        missing_nodes = [
            n for n in path_to_nodes[pid] if n not in nodes and n not in graph
        ]

        # dangling path: has nodes but no edges
        dangling_nodes = None
        if len(edges) == 0:
            dangling_nodes = list(path_to_nodes[pid])
